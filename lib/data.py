...
def get_stl_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1):
    """[summary]
    ...
    """
    # Convert train-test labels into numpy array.
    if isinstance(abn_cls_idx,str):
        abn_cls_idx = int(abn_cls_idx)
    trn_lbl = np.array(trn_lbl)
    tst_lbl = np.array(tst_lbl)
